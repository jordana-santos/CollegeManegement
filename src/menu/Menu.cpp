...
bool Menu::validateData(string data, string type) {
	if(type == "name"){		
		for(char c : data){
			if(isdigit(c)){
				cout<<" - Insira um nome válido, por gentileza."<<endl;
				return false;
			}
		}
	} else if(type == "phone"){
		if(data[0]!='(' || data[3]!=')' || data.length()!=15){
			cout<<" - Insira um número de telefone válido, por gentileza."<<endl;
			return false;
		}
	} else if(type == "RA"){
		...
		if(data.length() != 6 || notNumeric){
			cout<<" - Insira um RA válido, por gentileza."<<endl;
			return false;
		}
	}
	return true;
};
bool Menu::validateData(int data, string type) {
	if(type == "age"){
		if(data < 16 || data > 100){
			cout<<" - Insira uma idade válida, por gentileza."<<endl;
			return false;
		}
	} else if(type == "semester"){
		if(data < 1 || data > 2){
			cout<<" - Insira um semestre válido (1 ou 2), por gentileza."<<endl;
			return false;
		}
	} else if(type == "year"){
		if(data < 1950){
			cout<<" - Insira um ano válido, por gentileza."<<endl;
			return false;
		}
	}
	return true;
...
bool Menu::validateData(double grade) {
	if(grade<0 || grade>10){
		cout<<" - Insira uma nota válida (0 - 10), por gentileza."<<endl;
		return false;
	}
	return true;
}
